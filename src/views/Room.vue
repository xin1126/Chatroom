  ...
  setup() {
    ...
    const addData = () => {
      if (message.value !== '') {
        ...
        message.value = '';
        database.ref('publicRoom').child(roomId).push().set(addMessage);
    ...
    const editData = (id, strMessage, status) => {
      if (status) {
        ...
        database.ref('publicRoom').child(roomId).child(id).set(newData);
        editInput.value = '';
        editMessage.value = '';
      }
    };
    ...
    onMounted(() => {
      if (name.value) {
        ...
        database.ref('publicRoom').once('value', (snapshot) => {
          roomId = Object.keys(snapshot.val()).pop();
          enterChatroom[name.value] = { content: '', status: true };
          database.ref('publicRoom').child(roomId).push().set(enterChatroom);
        });
        ...
        ws = new WebSocket('wss://vast-hollows-31339.herokuapp.com/');
        database.ref().on('value', (snapshot) => {
          data.value = Object.entries(snapshot.val().publicRoom[routeId]);
